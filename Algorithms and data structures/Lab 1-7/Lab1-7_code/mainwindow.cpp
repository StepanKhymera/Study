...
_stdcall int WINAPI Sort_thr(LPVOID param){
...
if(ui->checkBox->isChecked()){
    ...
    time->append("5 ms", 5);
    time->append("10 ms", 7);
    time->append("50 ms", 9.5);
    time->append("100 ms", 12);
    time->append("500 ms", 15);
    time->append("1 s", 18);
    time->append("10 s", 21);
    time->append("50 s", 24);
    time->append("100 s", 27);
...
//            std::vector<int> run;
            ...
            HANDLE h_thread;
            alg = i;
            h_thread = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)Sort_thr, Sorts,0,0);
//            Sorts[i]->sort(run);
            ...
            if(ui->checkBox->isChecked()) lines[i].append( v+(1)/10.0, toChartTime(times[v][i]));
            qApp->processEvents();
        }
    }
    Time_Out(times);
